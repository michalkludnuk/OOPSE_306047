...
int main() {
    fstream file;
file.open( "file.txt", ios::in | ios::out ); if( file.good() == true ) {
...
getline(cin, inscr); file.write(&inscr[0],inscr.length());
       //reading from file
...
char temp [10];
file.read(temp, 10);
cout << "another line from file: " << endl; for(int i=0; i<10; i++) cout <<temp[i]; cout << endl; 
file.close();
 }
